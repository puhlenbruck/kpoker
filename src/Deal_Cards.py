...
def straight_finder(in_cards):
    ...
    if strt_len >= 5:
        for ind in range(2,len(strt_vals)-2):
            if strt_vals[ind+2] == strt_vals[ind]+2 and strt_vals[ind-2] == strt_vals[ind]-2 and strt_vals[ind+1] == strt_vals[ind] + 1 and strt_vals[ind - 1] == strt_vals[ind] - 1:
                strt_flag = True
...
def read_hand(rem_hands, board):
    ...
    win_hands = {    'Four of a Kind': False,
                     'Full House': False,
                     'Flush': False,
                     'Straight': False,
                     'Set': False,
    ...
    win_hands['Pair'], win_hands['Two Pair'], win_hands['Set'], win_hands['Full House'], win_hands['Four of a Kind'] = match_finder(tot_vals_sort, tot_vals)
